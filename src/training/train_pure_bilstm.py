...
def main():
    parser = argparse.ArgumentParser(description="训练 Pure_BiLSTM 模型")
    ...
    # 步骤 2: 初始化模型
    # ==========================
    model_name = "Pure_BiLSTM"
    print(f"\n=============================================")
    print(f"  正在训练模型: {model_name}")
    print(f"=============================================")
    
    # 使用修改后的合适参数：hidden_dim=64, num_layers=2
    model = PureBiLSTM(num_classes=num_classes, hidden_dim=64, num_layers=2)
    
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    save_path = os.path.join(project_root, "checkpoints", "pure_bilstm.pth")
