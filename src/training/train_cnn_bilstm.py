...
def main():
    parser = argparse.ArgumentParser(description="训练 CNN_BiLSTM 模型")
    ...
    # 如果用户通过 --dataset 指定预定义数据集路径，则映射到相应目录（除非显式传入不同的 --data-root）
    if args.dataset is not None:
        ...
        # 如果用户没有手动改过 --data-root（与默认相同），则用映射路径
    ...
    train_loader, val_loader, test_loader, class_to_idx = get_dataloaders(data_root, batch_size=args.batch_size)
    ...
    model = CNN_BiLSTM(num_classes=num_classes, hidden_dim=64) # 对应 main.py 里加载时用的 `hidden_dim=64`
    
    # 修复相对路径引起的保存位置错乱问题
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    save_path = os.path.join(project_root, "checkpoints", "cnn_bilstm.pth")
