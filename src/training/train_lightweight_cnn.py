...
def main():
    parser = argparse.ArgumentParser(description="训练 Lightweight_CNN_BiLSTM 模型")
    ...
    # 步骤 2: 初始化模型
    # ==========================
    model_name = "Lightweight_CNN_BiLSTM"
    print(f"\n=============================================")
    print(f"  正在训练模型: {model_name}")
    print(f"=============================================")
    
    model = Lightweight_CNN_BiLSTM(num_classes=num_classes, hidden_dim=32) # 缩减隐藏层维度更轻量
    
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    save_path = os.path.join(project_root, "checkpoints", "lightweight.pth")
