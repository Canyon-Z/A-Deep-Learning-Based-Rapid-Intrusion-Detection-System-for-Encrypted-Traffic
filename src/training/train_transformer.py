...
def main():
    ...
    # 使用与 run_training_all.py 中一致的模型参数
    model = TrafficTransformer(input_dim=28, d_model=128, nhead=8, num_layers=4, num_classes=num_classes, dropout=0.05)
    
    # 修复相对路径引起的保存位置错乱问题
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    save_path = os.path.join(project_root, "checkpoints", "transformer.pth")
